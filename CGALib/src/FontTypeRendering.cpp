...
void FontTypeRendering::Initialize(const char* str) {
	// Se limpian las caras, buffers para textra y shaders.
	auto cleanup = [&]() {
		FT_Done_Face(face);
		FT_Done_FreeType(ft_lib);
	...
	// Se le indica a dicha cara el archivo ttf que se utilizar�.
	if (FT_New_Face(ft_lib, str, 0, &face) != 0) {
		std::cerr << "Unable to load BauhausBold.ttf\n";
		cleanup();
		return exit(-1);
	}
	// Inicializaci�n de los VBOs para la textura.
 ...
 */
void FontTypeRendering::render(const std::string &str, float x, float y, float red, float green, float blue, int size) {
